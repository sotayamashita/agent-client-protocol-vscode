...
interface PermissionItem extends vscode.QuickPickItem { optionId: string }
...
function ensureTrusted(): boolean {
  if (!vscode.workspace.isTrusted) {
    vscode.window.showWarningMessage('Workspace is not trusted. ACP connection is disabled.');
    return false;
  }
  return true;
}
...
function assertAbsInsideWorkspace(absPath: string): vscode.Uri {
  ...
  const isInside = normFile === normWs || normFile.startsWith(normWs + path.sep);
  if (!isInside) {
...
class VSCodeClient /* implements Client */ {
  ...
  async requestPermission(params: any): Promise<any> {
    ...
    const picked = await vscode.window.showQuickPick(items, { placeHolder: 'Grant permission?' });
    return picked
...
async function connectAgent(): Promise<void> {
  ...
  if (child && !currentSessionId) {
    output.appendLine('[connect] Detected running agent without session. Resetting...');
    await disconnectAgent(false);
  }

  output.appendLine(`[spawn] ${agentPath} ${agentArgs.join(' ')}`);
  const proc = spawn(agentPath, agentArgs, { stdio: ['pipe', 'pipe', 'pipe'], env: { ...process.env } });
  child = proc;
  proc.once('spawn', () => {
  ...
  connection = new ClientSideConnection(() => new VSCodeClient(output), toAgent, fromAgent);
  ...
  try {
    ...
    output.appendLine('[hint] エージェントが ACP/stdio を話していない可能性があります。acp.agentArgs に "--stdio" 等が必要か確認してください。');
    await disconnectAgent(false);
    vscode.window.showErrorMessage('ACP: initialize がタイムアウト/失敗しました。Output を確認してください。');
    return;
  }
  ...
  try {
    newSessionRes = await withTimeout(connection.newSession(newSession), 10000, 'newSession');
  } catch (e) {
    output.appendLine(`[rpc] newSession error: ${String(e)}`);
    await disconnectAgent(false);
    vscode.window.showErrorMessage('ACP: newSession がタイムアウト/失敗しました。Output を確認してください。');
    return;
  }
  currentSessionId = newSessionRes?.sessionId ?? currentSessionId;
  output.appendLine(`[rpc] newSession ok sessionId=${currentSessionId ?? 'n/a'}`);
...
async function sendPrompt(): Promise<void> {
  if (!connection || !currentSessionId) {
    vscode.window.showWarningMessage('ACP: Not connected. Run "ACP: Connect Agent" first.');
    return;
  }
  const prompt = await vscode.window.showInputBox({ prompt: 'Send prompt to agent' });
  if (!prompt) return;
  ...
  try {
    ...
    if (e && typeof e === 'object' && (e.code === -32000 || e.code === 'AuthenticationRequired')) {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('acp.disconnect', () => disconnectAgent(true)),
    vscode.commands.registerCommand('acp.reconnect', async () => { await disconnectAgent(false); await connectAgent(); }),
    vscode.commands.registerCommand('acp.setAgentArgs', async () => {
      const current = vscode.workspace.getConfiguration('acp').get<string[]>('agentArgs') ?? [];
      ...
      try {
        ...
        args = input.match(/\".*?\"|\S+/g)?.map(s => s.replaceAll('"','')) ?? [];
      }
      await vscode.workspace.getConfiguration('acp').update('agentArgs', args, vscode.ConfigurationTarget.Global);
      vscode.window.showInformationMessage(`ACP: agentArgs set to ${JSON.stringify(args)}`);
    }),
    ...
    vscode.commands.registerCommand('acp.setAgentPath', async () => {
      ...
      await vscode.workspace.getConfiguration('acp').update('agentPath', fsPath, vscode.ConfigurationTarget.Global);
