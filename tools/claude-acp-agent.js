...
for (const a of argv) {
  if (a.startsWith("--claude-path="))
    CLAUDE_PATH = a.slice("--claude-path=".length);
    ...
    switch (method) {
      ...
      case "session/prompt": {
        ...
        proc.stderr.on("data", (b) => {
          log("[claude/stderr]", b.toString("utf8").trim());
        });
          ...
          send({
            ...
            params: {
              ...
              update: {
                ...
                content: { type: "text", text: String(textOut) },
              },
            },
          });
          // Finish
