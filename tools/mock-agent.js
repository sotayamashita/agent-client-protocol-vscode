...
function send(msg) {
  ...
  try { process.stderr.write(`[mock] send ${s}`); } catch {}
...
function handleRequest(req) {
  ...
  switch (method) {
    case 'initialize': {
      return send({ jsonrpc: '2.0', id, result: { protocolVersion: 1, agentCapabilities: {}, authMethods: [] } });
    }
    case 'session/new': {
      return send({ jsonrpc: '2.0', id, result: { sessionId: 'dev-session-1' } });
    }
    case 'session/prompt': {
      const sessionId = params?.sessionId ?? 'dev-session-1';
      const text = Array.isArray(params?.prompt)
        ? params.prompt.filter(b => b && b.type === 'text').map(b => b.text).join(' ')
        : '';
...
rl.on('line', (line) => {
  try { process.stderr.write(`[mock] recv ${line.length} bytes\n`); } catch {}
  const s = line.trim();
  if (!s) return;
  let msg;
  try { msg = JSON.parse(s); } catch (e) {
    try { process.stderr.write(`[mock] parse error: ${String(e)}\n`); } catch {}
    return; // ignore parse errors for simplicity
  }
  if (msg && msg.method && typeof msg.method === 'string') {
    // request or notification
    if (Object.prototype.hasOwnProperty.call(msg, 'id')) {
      try { process.stderr.write(`[mock] request ${msg.method}\n`); } catch {}
      handleRequest(msg);
    } else {
